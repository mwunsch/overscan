...
@hyperlink["https://gstreamer.freedesktop.org"]{GStreamer} is an open source framework for creating streaming media applications. More precisely it is ``a library for constructing graphs of media-handling components.'' GStreamer allows for the creation of media processing pipelines, and is the core of the multimedia capabilities of Overscan. GStreamer is written in the C programming language with the GLib Object model. This module, included in the Overscan package, provides Racket bindings to GStreamer; designed to provide support for building media pipelines in conventional, idiomatic Racket without having to worry about the peculiarities of C.
